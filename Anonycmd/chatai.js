...
function loadUserChataiData() {
    try {
        ...
        console.error('⚠️ Error loading chatai data:', error.message);
        return { chatai: {} };
    }
}
...
function saveUserChataiData(data) {
    try {
        ...
        console.error('⚠️ Error saving chatai data:', error.message);
    }
}
...
async function handleChataiCommand(sock, chatId, message, match) {
    ...
    if (chatId.endsWith('@g.us')) {
        await showTyping(sock, chatId);
        return sock.sendMessage(chatId, {
            text: '⚠️ Chatai is only available in private chats. Message me directly to use this feature.',
            quoted: message
        });
    }
    ...
    if (isOwner) {
        ...
        if (senderId !== chatId) {
            await showTyping(sock, chatId);
            return sock.sendMessage(chatId, {
                text: '⚠️ You can only enable/disable chatai for your own chat.',
                quoted: message
            });
        }
...
async function handleChataiResponse(sock, chatId, message, userMessage, senderId) {
    ...
    try {
        ...
        console.error('⚠️ Chatai error:', error);
