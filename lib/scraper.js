  ...
  download: async (link, format) => {
    ...
    if (!savetube.isUrl(link)) {
      return {
        status: false,
        code: 400,
        error: "astagaa, itu bukan link YouTube njrr"
      };
    }
    ...
    if (!format || !allFormats.includes(format)) {
      return {
        status: false,
        code: 400,
        error: "woii, itu bukan format yang ada woilah, liat dibawah ini njrr",
    ...
    if (!id) {
      return {
        status: false,
        code: 400,
        error: "Yaelah broo, link mya ada yg salah tuh astaga"
      };
    }
