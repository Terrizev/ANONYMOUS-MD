        ...
        if (isBanned(senderId) && !userMessage.startsWith('.unban')) {
            ...
            if (Math.random() < 0.1) {
                await sock.sendMessage(chatId, {
                    text: '⚠️You are banned from using the bot.',
                    ...channelInfo
                });
            }
            return;
    ...
    break;
            ...
            case userMessage === '.tagall':
                if (isSenderAdmin || message.key.fromMe) {
                    ...
                    await sock.sendMessage(chatId, { text: 'Sorry, For group admins only to use .tagall command.', ...channelInfo });
                }
                break;
            ...
            case userMessage.startsWith('.antilink'):
                if (!isGroup) {
                    await sock.sendMessage(chatId, {
                        text: 'This command can only be used in groups⚠️.',
                        ...channelInfo
                    });
                    return;
                }
            ...
            case userMessage.startsWith('.weather'):
                ...
                if (city) {
                    ...
                    await sock.sendMessage(chatId, { text: 'Please specify a city, e.g., .weather Kampala', ...channelInfo });
                }
                break;
            ...
            case userMessage.startsWith('.welcome'):
                if (isGroup) {
                    ...
                    if (isSenderAdmin || message.key.fromMe) {
                        ...
                        await sock.sendMessage(chatId, { text: 'Sorry, only group admins can use this command.⚠️', ...channelInfo });
                    }
                } else {
                    await sock.sendMessage(chatId, { text: 'This command can only be used in groups.⚠️', ...channelInfo });
                }
                break;
            ...
            case userMessage.startsWith('.antibadword'):
                ...
                if (!isBotAdmin) {
                    await sock.sendMessage(chatId, { text: '*I must be admin to use this feature*', ...channelInfo });
                    return;
                }
...
case userMessage === '.clearsession' || userMessage === '.clearsesi':
    ...
    break;
        ...
        console.error('⚠️Error in message handler:', error.message);
        // Only try to send error message if we have a valid chatId
        if (chatId) {
            await sock.sendMessage(chatId, {
                text: '⚠️Failed to process command!',
                ...channelInfo
            });
        }
    }
